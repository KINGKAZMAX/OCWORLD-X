...
export default function HomeScreen() {
  ...
  // 课程详情弹窗状态
  const [activeSlotDetail, setActiveSlotDetail] = useState<TodayCourse | null>(
    null,
  );
  ...
  const fetchTodaySchedule = useCallback(async () => {
    ...
    try {
      ...
      const accountResponse = await fetch(
        `${API_BASE_URL}/api/bindaccount/jwxt/with-password`,
        {
          ...
          if (course.skbj) {
            try {
              const studentResponse = await fetch(
                `${SCHEDULE_API_URL}/api/schedule/userinfo`,
                {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
                  ...
                  <View style={styles.courseHeader}>
                    <View style={styles.courseTimeRow}>
                      ...
                      <Text style={styles.courseTime}>
                        {todayWeekLabel} {course.jcxx}节
                      </Text>
                    </View>
                    {course.skbj &&
                      studentCounts[course.skbj] !== undefined && (
                        <Text style={styles.courseStudentCount}>
                          {studentCounts[course.skbj]}人
                        </Text>
                      )}
                  </View>
        ...
        >
          <View style={styles.detailModalContainer}>
            <Pressable
              ...
              onPress={() => setActiveSlotDetail(null)}
            />
              ...
              >
                <View style={styles.detailCourseCard}>
                  {[
                    ...
                    {
                      label: "节次",
                      value: activeSlotDetail.jcxx
                        ? `第${activeSlotDetail.jcxx}节`
                        : undefined,
                    },
...
function EmptyScheduleIcon() {
  return (
    <Svg width={80} height={80} viewBox="0 0 100 100" fill="none">
      ...
      {/* 装饰星星 */}
      <Path
        d="M82 18l2 4 4 0.5-3 2.5 0.8 4-3.8-2-3.8 2 0.8-4-3-2.5 4-0.5z"
        fill="#fbbf24"
      />
