...
export default function FavoriteSeatsScreen() {
  ...
  const fetchCollectData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/study-room/collecttime`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          { text: "再想想", style: "cancel" },
          {
            ...
            onPress: async () => {
              ...
              try {
                ...
                const response = await fetch(
                  `${API_BASE_URL}/api/study-room/cancel`,
                  {
                    method: "POST",
                    headers: {
                      Authorization: `Bearer ${token}`,
                ...
                if (response.ok && data.code === 0) {
                  ...
                  const msg =
                    data.message || data.msg || data.detail || "取消预约失败";
          ...
          { text: "取消", style: "cancel" },
          {
            ...
            onPress: async () => {
              ...
              try {
                ...
                const day = getTodayDate();
                const response = await fetch(
                  `${API_BASE_URL}/api/study-room/seat`,
                  {
                    method: "POST",
                    headers: {
                      Authorization: `Bearer ${token}`,
                ...
                if (response.ok && data.code === 0) {
                  ...
                  const msg =
                    data.message || data.msg || data.detail || "预约失败";
  ...
  const renderSeatItem = (item: SeatItem, index: number) => {
    ...
    const date = oftenDate || getTodayDate();
    const time = oftenTime || "08:00-22:30";

    return (
      <View key={item.space_id || item.id || index} style={styles.seatCardNew}>
        <View style={styles.seatCardContent}>
          <View style={styles.seatCardRow}>
            <Text style={styles.seatCardLabel}>日期</Text>
            <Text style={styles.seatCardValue}>{date}</Text>
          </View>
          <View style={styles.seatCardRow}>
            <Text style={styles.seatCardLabel}>时间</Text>
            <Text style={styles.seatCardValue}>{time}</Text>
          </View>
          <View style={styles.seatCardRow}>
            <Text style={styles.seatCardLabel}>地点</Text>
            <Text style={styles.seatCardValue} numberOfLines={1}>
              {location}
            </Text>
          </View>
          <View style={styles.seatCardRow}>
            <Text style={styles.seatCardLabel}>座位</Text>
            <Text style={styles.seatCardValueHighlight}>{seatNumber}</Text>
          </View>
        </View>
        <Pressable
          style={styles.seatCardButton}
          onPress={() => handleReserve(item)}
        >
          <Text style={styles.seatCardButtonText}>预约</Text>
        </Pressable>
      </View>
    );
  };
  ...
  const renderSubscribeItem = (item: SubscribeItem, index: number) => {
    ...
    const showTime =
      item.showTime || `${item.beginTime || ""} 至 ${item.endTime || ""}`;
      ...
      <View key={item.id || index} style={styles.subscribeCard}>
        <View style={styles.subscribeHeader}>
          ...
          <View style={styles.subscribeTypeBadge}>
            <Text style={styles.subscribeTypeText}>
              {item.statusname || "普通座位"}
            </Text>
          </View>
        </View>
        ...
        {subscribeData.length > 0 && (
          <View style={styles.subscribeSection}>
            {subscribeData.map((item, index) =>
              renderSubscribeItem(item, index),
            )}
          </View>
        )}
        ...
        <View style={styles.seatSection}>
          ...
          <View style={styles.tabContainer}>
            <Pressable
              style={[
                styles.tabItem,
                activeTab === "collect" && styles.tabItemActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.tabText,
                  activeTab === "collect" && styles.tabTextActive,
                ]}
              >
                收藏
              </Text>
            ...
            <Pressable
              style={[
                styles.tabItem,
                activeTab === "often" && styles.tabItemActive,
              ]}
            ...
            >
              <Text
                style={[
                  styles.tabText,
                  activeTab === "often" && styles.tabTextActive,
                ]}
              >
                常用
              </Text>
